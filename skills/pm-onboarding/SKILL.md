...
Read `.harness/references/data-model.md` before assigning IDs or statuses. Use the agent-only initialization contract in `.harness/references/automation.md`. Populate the project files, create an initial schedule only from confirmed dates, update `project/status.md` and `memory/current.md`, then rebuild and lint.
