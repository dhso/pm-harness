...
通过类型化记录操作应用计划变更。一次逻辑变更涉及多个任务或里程碑时，先查 `schedule.batch-upsert` 契约并一次预演、一次提交，不要循环调用 `schedule.upsert`；单条更新仍使用 `schedule.upsert`。初始基线先整体写成草稿，再调用一次 `schedule.baseline.approve`。确保基线修订、摘要、审批和变更日志保持一致。执行重建，使 `project/gantt.md` 反映规范数据。记录重要的计划决定和未解决依赖。
