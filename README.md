...
原始资料与结构化事实分开保存，以控制工作区体积、重复内容和二进制噪声。对外发送正文或附件前，AI Agent 会提醒检查凭证、Token、私钥和 `.env` 等敏感内容。
...
| `.harness/lib/compensation.mjs` | 补偿快照、冲突检查和最近操作撤销 |
...
| `tests/compensation.test.mjs` | 覆盖撤销、补偿护栏、归档保留、稳定 ID、CLI 和并发写锁 |
