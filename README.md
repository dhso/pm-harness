...
一个面向项目管理人员、由 AI Agent 在本地工作区运行的项目协作模板。

项目经理只需通过自然语言描述目标、提问、提供邮件或截图、汇报进展以及要求制作交付物。AI Agent 在后台维护项目计划、甘特图、需求、风险、知识库、记忆、活动记录、交付物和索引，并在重要操作后进行一致性检查。

这里的“自动维护”发生在每次 AI Agent 对话内；Harness 不启动定时器、文件监听器或常驻后台服务。
...
- 沉淀可追溯的项目 Wiki、决策和经验；
- 在不同 AI Agent 会话之间延续项目上下文；
- 在不要求项目经理理解 Git 的前提下保留可追溯操作，并能安全撤销最近的普通事实写入。
...
1. 为项目复制一份本目录。
2. 在支持本地工作区的 AI Agent 客户端中打开复制后的目录。
3. 对 AI Agent 说：`初始化这个项目`。
4. 根据引导提供项目名称、目标、成功标准、范围、干系人、已知时间点和预期交付物。
5. 此后直接用自然语言开展项目工作。

项目经理不需要运行命令、编辑 JSON、维护索引或手工更新甘特图。`.harness/` 中的内部工具只供 AI Agent 自动维护本地文件。
...
来源材料首先被视为证据，不会自动变成已经批准的决定。发送者、时间、原话、权威性或含义不明确，并且会影响负责人、日期、需求或承诺时，AI Agent 会提出一个聚焦问题。

工作过程中如果发现关键信息不清、证据不足或存在多种合理解释，AI Agent 会及时暂停受影响的步骤并向你澄清。对于会影响后续任务路径的未明确决定，会列出必要选项、影响和取舍，请你选择后再继续；不改变方向的低影响细节才会采用并标注保守假设。
...
文件生成、批准、发送和验收是不同状态。AI Agent 不会因为文件已经生成就把它记录为已交付或已验收；批准后还会用文件 SHA-256 检测原地覆盖。
...
文件是长期事实源；AI Agent 的会话记忆只是辅助。重要信息会被整理进项目文件，而不是依赖某一次聊天上下文。

## 目录总览

```text
pm-harness/
├── AGENTS.md                  AI Agent 的项目级工作规则
├── README.md                  本使用说明
...
├── .gitignore                 供选择 Git 的维护者使用的排除策略
├── .gitattributes             供选择 Git 的维护者使用的文件属性
...
| 路径 | 作用 | 存储方式 |
|---|---|---|
| `knowledge/sources.json` | 邮件、聊天、截图、文档等来源的结构化登记，保存来源 ID、摘要、时间、发送者、定位信息和哈希 | 结构化事实 |
| `knowledge/inbox.json` | 从来源中提取、尚待处理的事实、反馈、请求、决定、承诺、行动、风险、问题、疑问和假设 | 结构化事实 |
| `knowledge/catalog.json` | `WIKI-###`、页面路径、状态、来源、关联对象与复查日期的事实源 | 结构化事实 |
| `knowledge/wiki/` | 按主题整理并带来源追溯的长期项目知识 | 长期文档 |
| `knowledge/index.md` | Wiki 页面导航 | 自动生成视图 |
...
交付物也可以进入 `superseded` 或 `cancelled`。已批准文件不得原地覆盖；正式修订会建立新版本和新的 `DEL-###`，并通过替代关系保留版本链。不再需要日常使用的旧版本移入本地归档。
...
规则不会自动生效。只有你明确批准后，AI Agent 才会更新有效规则，并在后续复查它是否应该保留、收窄、修订或退役。

## `archive/`：本地原始资料归档

| 路径 | 作用 | 存储方式 |
|---|---|---|
| `archive/files/` | 原始邮件、聊天导出、截图、附件、历史草稿和旧版二进制文件 | 本地原件 |
| `archive/index.json` | 归档记录 ID、逻辑对象、文件名、类型、SHA-256、归档时间、原因、替代关系和可用状态 | 结构化事实 |

每个归档条目使用 `ARC-###`。即使原件只保留在本机，仍可从结构化项目记录追溯到归档索引，并在原件存在时进行核验。

原始资料与结构化事实分开保存，以控制工作区体积、重复内容和二进制噪声。凭证、Token、Cookie、验证码、私钥和 `.env` 文件不得进入项目资料。
...
模板是内容脚手架，不要求机械填满所有字段。AI Agent 会根据实际受众和用途进行调整。
...
该目录面向 AI Agent，而不是项目经理。

| 路径 | 作用 |
|---|---|
| `.harness/config.json` | 时区、近期窗口、任务陈旧阈值、重复观察阈值和归档目录等配置 |
| `.harness/references/` | 来源摄取、计划、变更、交付、文件边界和规则演进的详细政策 |
...
| `.harness/lib/compensation.mjs` | Git-free 补偿快照、冲突检查和最近操作撤销 |
| `.harness/lib/views.mjs` | 甘特图、活动、知识、交付物和规则生成视图 |
| `.harness/lib/lint.mjs` | 数据、生命周期、文件、补偿快照和模块边界的只读检查 |
| `.harness/lib/environment.mjs` | Skills、记忆、软链接和模块规模环境审计 |
...
结构化 Store 和操作日志会持续增长。Harness 会在超过配置大小阈值时提示；读取优先使用按字段、按条件和按数量限制的查询，不把完整 JSON 复制到对话。维护不会静默删除事实或审计记录；不可直接补偿的长文档也不会再复制整份事实快照。
...
| `AGENTS.md` | AI Agent 每次进入项目时遵循的总工作契约和批准边界 |

## 文件保留与外部版本控制

| 类别 | Harness 默认策略 |
|---|---|
| Harness 指令、Skills、模板和检查规则 | 长期保留 |
| 项目目标、需求、计划、风险、决定和状态 | 作为事实源长期保留 |
| 结构化来源、必要引用、哈希和 Wiki | 长期保留 |
| 当前交付物和仍有效的批准终稿 | 长期保留 |
| 甘特图、知识库和交付物索引 | 可重新生成 |
| 精简活动记录、记忆和有效规则 | 长期保留 |
| 原始邮件、完整聊天导出、截图和附件 | 仅保留在本地归档 |
| 历史草稿、旧二进制和临时渲染 | 归档或删除 |
| 缓存、备份、完整模型对话和工具日志 | 不作为项目事实 |
| 凭证、密钥、Token、Cookie 和 `.env` | 禁止写入项目资料 |

项目经理无需感知 Git。Harness 不安装、初始化或调用 Git，不自动提交，也不操作分支和远端。团队如需版本控制或备份，可以在 Harness 之外独立配置，不会改变 Harness 的运行逻辑。

需要撤销时直接说“撤销刚才那步”。AI Agent 会先展示将恢复的事实和保留的归档原件，确认后追加一条补偿操作。原操作不会消失；只能依次撤销最近的普通写入，已批准内容和受控变更必须走正向变更流程。

## 哪些变化需要你的确认

AI Agent 可以自动执行低风险的本地整理，例如登记来源、添加证据说明、更新 Wiki、记录活动、更新预测进度、创建草稿和重建索引。
...
AI Agent 会结合近期活动、当前计划、逾期和阻塞任务、依赖、近期里程碑、风险、待决审批、未处理来源、待交付事项及 Wiki/规则复查，给出不超过三个带原因的首要行动。

### 收到新信息时

AI Agent 会先登记来源和证据，再更新低风险事实；可能改变需求或承诺的内容进入变更提案，不直接覆盖原记录。

### 汇报进展时

AI Agent 会更新任务状态、预测/实际时间、进度、证据和下一步，并在活动记录中写明发生时间和结果。

### 完成重要工作后

AI Agent 会更新相关项目记录、Wiki、记忆、计划和交付索引，重建自动生成文件并执行一致性检查，最后只汇报关键结果、时间变化、知识沉淀和仍需批准的事项。

### 复盘或出现重复问题时

AI Agent 会先积累观察证据；达到重复阈值后形成有评价指标和复查时间的规则提案，待你批准后进入有效规则。
...
不需要。目录结构是 AI Agent 的长期工作空间。你可以检查其中的文档，但日常只需通过对话推进项目。
...
不建议。应告诉 AI Agent 要修改的任务、日期或依赖，由它更新 `project/schedule.json` 并重新生成 `project/gantt.md`。

### 为什么来源原件与项目事实分开保存？

邮件、截图、附件和旧版本通常体积较大、重复且难以比较。Harness 将原件留在本地归档，同时把必要摘要、引用、哈希、关系和索引保存为结构化项目事实。

### 换一台电脑后归档原件不存在怎么办？

项目事实和归档索引仍然可用，一致性检查会把原件标记为当前机器不可用，而不会把已经记录的来源事实静默删除。需要核验原件时再恢复对应文件。

### AI Agent 会自动接受客户提出的变化吗？

不会。邮件、聊天或截图只是证据。影响目标、范围、预算、承诺日期、需求或验收的内容必须经过权威性判断和变更确认。

### 想撤销刚才那次改动怎么办？

直接说“撤销刚才那步”。AI Agent 会展示将恢复的事实，确认后追加一条补偿操作；原操作和归档原件仍然保留。只能依次撤销最近的可撤销操作；更早的改动通过正向修正处理，已批准的内容走变更请求。

### 团队自己使用 Git 时需要注意什么？

这是 Harness 之外的团队策略。仓库内提供了保守的 `.gitignore` 和 `.gitattributes` 默认值；原始归档和旧二进制建议因体积与噪声排除，凭证、密钥、Token、Cookie、验证码和 `.env` 始终不应进入版本历史。
